...
class ItemRanker:
    ...
    def __init__(self, n, hidden=64, seed=42):
        ...
        self.W1 = rng.uniform(-0.1, 0.1, size=(hidden, n))
        self.W2 = rng.uniform(-0.1, 0.1, size=(hidden,))
        self.b = 0.0
    ...
    def forward(self, x):
        ...
        self.out_raw = self.W2 @ self.h + self.b
        self.out = self.sigmoid(self.out_raw)
    ...
    def backward(self, y_true, lr=0.01):
        ...
        d_hidden = self.W2 * d_out
        d_hidden[self.z1 <= 0] = 0

        dW1 = np.outer(d_hidden, self.x)
...
def encode_build(build):
    ...
    for item in build:
        ...
        if item == 0:
            result.extend([0.0] * MAX_ITEMS)
        else:
            result.extend(encode_item(item))
...
def create_samples_with_negatives(match, negatives=1):
    ...
    for player in match:
        ...
        for real_item in real_items:
            ...
            x_pos = context + encode_build(pad_build(partial)) + encode_item(real_item)
            samples.append((x_pos, 1))
            ...
            for _ in range(negatives):
                ...
                x_neg = context + encode_build(pad_build(partial)) + encode_item(fake_item)
                samples.append((x_neg, 0))

            partial.append(real_item)
...
def main():
    parser = argparse.ArgumentParser(description='Train or run recommendations using item ranker')
    parser.add_argument('--train', action='store_true', help='Uruchom trening modelu')
    parser.add_argument('--recommend', action='store_true', help='Uruchom rekomendację przy użyciu istniejącego modelu')
    ...
    parser.add_argument('--model', default='./app/model.json', help='Ścieżka do modelu (wejście dla --recommend)')
    ...
    parser.add_argument('--sample', type=str, default=None, help='Przykładowy kontekst jako JSON (używany z --recommend)')
    args = parser.parse_args()

    # If user requested training
    if args.train:
    ...
    # If user requested recommendation
    if args.recommend:
        # load model
        try:
            model = ItemRanker.load(args.model)
        except Exception as e:
            print(f'Nie można załadować modelu z {args.model}: {e}')
            return
        ...
        if args.sample:
            try:
                ...
                print(f'Błąd parsowania --sample: {e}')
                return
        else:
            # default sample (similar to run_debug_recommend/run_probe2)
        ...
        build = recommend(model, context)
        
        print('Zalecany build:')
        for item_id in build:
            item_name = item_id_to_name.get(item_id, "Unknown Item")
            print(f"{item_id} -> {item_name}")
        return

    # If neither flag specified, show help
    parser.print_help()
...
if __name__ == '__main__':
    ...
    import uv
    import sys
    uv.run(["python", "app/siec.py"] + sys.argv[1:])
    """